...
async function authRoutes(fastify) {
    ...
    fastify.post('/login', async (request, reply) => {
        ...
        const isValid = (0, hash_1.comparePassword)(password, user.passwordHash);
        if (!isValid) {
    ...
    fastify.post('/register', async (request, reply) => {
        ...
        const userRole = await database_1.prisma.role.findUnique({ where: { name: 'USER' } });
        if (!userRole) {
        ...
        await (0, queue_1.addEmailJob)('send-welcome', { email, name });
        await (0, audit_1.createAuditLog)({
            userId: newUser.id,
    ...
    fastify.post('/forgot-password', async (request, reply) => {
        ...
        if (user) {
            ...
            await (0, queue_1.addEmailJob)('send-reset-password', { email, resetUrl });
        }
